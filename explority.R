...
print("--- Summary of Placement Distribution by Year ---")
print(summary_report)

print("--- Frequency of Status Combinations per Person (All Years) ---")
table(placement_dist$unique_status_rows)

top_rows <- placement_dist[order(-unique_status_rows)][1:10]
print("Top 10 individuals with the most status rows:")
print(top_rows)
...
print("Table showing how many people have 1 row, 2 rows, etc:")
print(row_counts_summary)
...
summary(dt_unbiased[, .(a1, a2, a3, a4, a5, a6, a7, a8)])

dt_unbiased[,
  combo_count_robust := rowSums(.SD > 0, na.rm = TRUE),
  .SDcols = state_cols
]
table(dt_unbiased$combo_count_robust)

top_ids <- placement_dist[unique_status_rows >= 7, id]

complex_persons <- dt_unbiased[id %in% top_ids]
print(complex_persons[, .(id, combo_count, total_np, a1, a2, a4, a5, a7, a8)])
...
complex_full_profile <- merge(
  complex_full_profile,
  regB_counts,
  by = c("id", "year"),
  all.x = TRUE
)

complex_full_profile[is.na(complex_full_profile)] <- as.double(0)
complex_full_profile
...
complex_full_profile <- dt_unbiased[
  id %in% top_ids,
  .(id, year, combo_count, total_np, a1, a2, a4, a5, a7, a8)
]
...
print("Full Profile for Complex IDs (10 Regional Columns + Status Combos):")
print(complex_full_profile)
...
table(region_counts_per_id$n_regions)
dt1b[,
...
modsum <- summary(modout$model)
summary(modout$model)
modout1a <- matchit(
  treat ~ ag + sg + yr + rg,
  data = orcmatch_temp,
  distance = "gam",
...
#--------------------------------------------
save.image("finne_env.RData")
saveRDS(as.list(.GlobalEnv), "finne_env.rds")
